...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/07/05 20:30:49 by kcrius            #+#    #+#             */
/*   Updated: 2022/07/05 20:30:51 by kcrius           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	print_act(char *s, t_phil_process *pprocess, int isdead)
{
	...
	print_unsigned_number(get_time_to_mls() - data->start, 10, 'a');
	write(1, " ", 1);
...
void	eating(t_phil_process *pprocess, t_data *data)
{
	...
	pprocess->last_eat = get_time_to_mls();
...
void	*death_and_life_cheack(void	*argc)
{
	...
	{
		sem_wait(pprocess->last_eating);
		if (get_time_to_mls() - pprocess->last_eat \
...
int	act(t_phil_process *pprocess)
{
	...
	death_and_life_cheack, pprocess) != 0)
		exit(error("thread create error\n"));
